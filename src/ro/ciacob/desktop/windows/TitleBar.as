package ro.ciacob.desktop.windows
{
	...
	public class TitleBar extends UIComponent
	{
		 ...
		 *  @productversion Flex 3
		 */
		public var closeButton:WindowButton;
		 ...
		 *  @productversion Flex 3
		 */
		public var maximizeButton:WindowButton;
		 ...
		 *  @productversion Flex 3
		 */
		public var minimizeButton:WindowButton;
		...
		override protected function createChildren():void
		{
			...
			if (!minimizeButton) {
				minimizeButton = new WindowButton();
			...
			if (!maximizeButton)
			{
				maximizeButton = new WindowButton();
		...
		override protected function measure():void {
			...
			measuredHeight = Math.max (
				titleTextField.textHeight + UITextField.TEXT_HEIGHT_PADDING,
				Math.max (maximizeButton.measuredHeight, minimizeButton.measuredHeight, closeButton.measuredHeight) + 12
			);
		...
		override protected function updateDisplayList (unscaledWidth:Number, unscaledHeight:Number) : void {
			...
			if (titleBarBackground) {
				...
				IFlexDisplayObject(titleBarBackground).setActualSize (unscaledWidth, unscaledHeight);
			}
			...
			if (titleIconObject) {
				...
				leftOffset += cornerOffset + titleIconObject.width + getStyle("buttonPadding");
			}
			...
			if (buttonAlign == "right") {
				placeButtons (buttonAlign, width, height, leftOffset, rightOffset, cornerOffset);
			} else if (buttonAlign == "left") {
				placeButtons (buttonAlign, width, height, leftOffset, rightOffset, cornerOffset);
			} else {
				if (isMac()) {
					buttonAlign = "left";
					placeButtons ("left", width, height, leftOffset, rightOffset, cornerOffset);
				} else {
					placeButtons ("right", width, height, leftOffset, rightOffset, cornerOffset);
				}
			}
			
			// Position the title
			var titleAlign:String = String(getStyle("titleAlignment"));
		...
		protected function placeButtons (align : String,
			...
			else
			{
				...
				closeButton.move (edgePad, (unscaledHeight - closeButton.measuredHeight) / 2);
				minimizeButton.move (pad + edgePad + closeButton.measuredWidth, (unscaledHeight - minimizeButton.measuredHeight) / 2);
				maximizeButton.move (edgePad + (pad * 2) + closeButton.measuredWidth + minimizeButton.measuredWidth,
					(unscaledHeight - maximizeButton.measuredHeight) / 2);
			}
		}
		...
		protected function placeTitle (titleAlign : String, leftOffset : Number, rightOffset : Number, buttonAlign : String) : void {
			...
			// titleAlign is center
				...
				if (buttonAlign == "left") {
					var titleWidth : Number = (width - rightOffset - buttonsX - 2);
					titleTextField.setActualSize (titleWidth, measureChromeText(titleTextField).height + UITextField.TEXT_HEIGHT_PADDING);
					titleX = Math.max (buttonsX, (width - leftOffset - rightOffset - textWidth) / 2);
					titleTextField.move (titleX,
						(height - (measureChromeText(titleTextField).height + UITextField.TEXT_HEIGHT_PADDING)) / 2);
				} else {
					titleTextField.setActualSize (
